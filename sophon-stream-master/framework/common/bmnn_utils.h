...
class BMNNTensor {
 ...
 public:
  ...
  float* get_cpu_data() {
    ...
    m_cpu_data_is_mapped = false;
    auto alloc_zero_fp32 = [&]() -> float* {
      float* buffer = new float[count]();
      assert(buffer != nullptr);
      return buffer;
    };
    auto log_copy_failure = [&](const char* stage, bm_status_t status) {
      std::cerr << "[BMNNTensor] " << stage << " failed for tensor " << m_name
                << ", ret=" << status << ", fallback to zero buffer"
                << std::endl;
    };
    ...
    if (can_mmap) {
      if (m_tensor->dtype == BM_FLOAT32) {
        ...
        if (BM_SUCCESS == ret) {
          ...
          if (BM_SUCCESS == ret) {
            pFP32 = (float*)addr;
            m_cpu_data_is_mapped = true;
          } else {
            std::cerr << "[BMNNTensor] invalidate failed for tensor " << m_name
                      << ", fallback to d2s, ret=" << ret << std::endl;
            bm_status_t unmapRet = bm_mem_unmap_device_mem(
                m_handle, reinterpret_cast<void*>(addr),
                bm_mem_get_device_size(m_tensor->device_mem));
            if (BM_SUCCESS != unmapRet) {
              std::cerr << "[BMNNTensor] cleanup unmap failed for tensor "
                        << m_name << ", ret=" << unmapRet << std::endl;
            }
          }
        } else {
          std::cerr << "[BMNNTensor] mmap failed for tensor " << m_name
                    << ", fallback to d2s, ret=" << ret << std::endl;
        }
