...
class Filter_Imp {
 public:
  ...
  void set_polygon_mode(PolygonMode mode) { polygon_mode = mode; }
 ...
 private:
  ...
  int frame_count = 0;
  PolygonMode polygon_mode = PolygonMode::kRectangleInside;
  ...
  bool isRectangleInsidePolygon(std::vector<common::Point<int>>& rectangle,
                                const std::vector<common::Point<int>>& polygon);
  bool isRectangleOverlapPolygon(std::vector<common::Point<int>>& rectangle,
                                 const std::vector<common::Point<int>>& polygon);
};
class Filter : public ::sophon_stream::framework::Element {
 public:
  ...
  static constexpr const char* CONFIG_INTERNAL_POLYGON_MODE = "polygon_mode";
  static constexpr const char* CONFIG_INTERNAL_FORWARD_EMPTY_FRAME =
      "forward_empty_frame";
