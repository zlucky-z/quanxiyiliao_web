...
common::ErrorCode Filter::initInternal(const std::string& json) {
  ...
  do {
    ...
    auto forwardEmptyFrameIt =
        configure.find(CONFIG_INTERNAL_FORWARD_EMPTY_FRAME);
    if (forwardEmptyFrameIt != configure.end() &&
        forwardEmptyFrameIt->is_boolean()) {
      mForwardEmptyFrame = forwardEmptyFrameIt->get<bool>();
    }
    // 遍历 JSON 数组
    ...
    for (auto& filter_array : *rulesIt) {
      ...
      for (auto& filter : *FiltersIt) {
        ...
        auto polygonModeIt = filter.find(CONFIG_INTERNAL_POLYGON_MODE);
        if (polygonModeIt != filter.end() && polygonModeIt->is_string()) {
          std::string polygonMode = polygonModeIt->get<std::string>();
          if (polygonMode == "bottom_center") {
            Filter_Imp_.set_polygon_mode(Filter_Imp::PolygonMode::kBottomCenter);
          } else if (polygonMode == "any_overlap") {
            Filter_Imp_.set_polygon_mode(Filter_Imp::PolygonMode::kAnyOverlap);
          } else {
            Filter_Imp_.set_polygon_mode(
                Filter_Imp::PolygonMode::kRectangleInside);
          }
        }
...
common::ErrorCode Filter::doWork(int dataPipeId) {
  ...
  if (inputPorts.empty()) {
    IVS_ERROR("Filter has no input ports, element id: {0}", getId());
  ...
  if (mForwardEmptyFrame) {
    common::ErrorCode errorCode =
        pushOutputData(outputPort, outDataPipeId,
                       std::static_pointer_cast<void>(objectMetadata));
    if (common::ErrorCode::SUCCESS != errorCode) {
      IVS_WARN(
          "Forward empty frame fail, element id: {0:d}, output port: {1:d}, "
          "data: {2:p}",
          getId(), outputPort, static_cast<void*>(objectMetadata.get()));
    }
    return errorCode;
...
bool Filter_Imp::isOutsideWorkingHours(
  ...
  for (int i = 0; i < times.size(); i++) {
    std::int64_t timestamp =
        common::normalizeTimestampToMilliseconds(
            objectMetadata->mFrame->mTimestamp,
            common::currentTimeMilliseconds()) %
        (1000LL * 60 * 60 * 24);
    int channel_id = objectMetadata->mFrame->mChannelIdInternal;
...
bool Filter_Imp::isInPolygon(
  ...
  for (int j = 0; j < objectMetadata->mDetectedObjectMetadatas.size(); j++) {
    ...
    for (int i = 0; i < areas.size(); ++i) {
      if (polygon_mode == PolygonMode::kBottomCenter) {
        common::Point<int> bottomCenter(bottom, (left + right) / 2);
        flag |= isPointInsidePolygon(bottomCenter, areas[i].points);
      } else if (polygon_mode == PolygonMode::kAnyOverlap) {
        flag |= isRectangleOverlapPolygon(rectangle, areas[i].points);
      } else {
...
bool Filter_Imp::isRectangleOverlapPolygon(
    std::vector<common::Point<int>>& rectangle,
    const std::vector<common::Point<int>>& polygon) {
  ...
  int rectTop = rectangle[0].mX;
  int rectBottom = rectangle[2].mX;
  int rectLeft = rectangle[0].mY;
  int rectRight = rectangle[1].mY;
  for (const common::Point<int>& point : polygon) {
    if (point.mX >= rectTop && point.mX <= rectBottom &&
        point.mY >= rectLeft && point.mY <= rectRight) {
      return true;
    }
  }
