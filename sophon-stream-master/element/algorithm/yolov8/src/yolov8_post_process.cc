...
namespace {
constexpr int kHumanChannelIdPrimary = 20;
constexpr int kHumanChannelIdSecondary = 10;
constexpr int kGpioDetectedValue = 0;  // Keep aligned with celectronicfence API semantics.
constexpr int kGpioIdleValue = 1;
const char* kHumanStateFileChannel20 = "/tmp/cef_human_detect_ch20.state";
const char* kHumanStateFileChannel10 = "/tmp/cef_human_detect_ch10.state";

int loadForcedChannelIdFromEnv() {
  static bool initialized = false;
  static int forcedChannelId = -1;
  if (initialized) {
    return forcedChannelId;
  }
  initialized = true;

  const char* rawValue = std::getenv("CEF_CHANNEL_ID");
  if (!rawValue) {
    return forcedChannelId;
  }

  try {
    int parsed = std::stoi(std::string(rawValue));
    if (parsed == kHumanChannelIdPrimary || parsed == kHumanChannelIdSecondary) {
      forcedChannelId = parsed;
    }
  } catch (...) {
  }
  return forcedChannelId;
...
int findPersonClassId(const std::shared_ptr<Yolov8Context>& context) {
  if (!context) {
    return -1;
  }
  for (size_t i = 0; i < context->class_names.size(); ++i) {
    if (context->class_names[i] == "person") {
      return static_cast<int>(i);
    }
  }

  // 兼容旧模型/自定义类别文件：当未显式命名 "person" 时，回退到 class 0（历史工程约定）。
  return 0;
}

void writeHumanState(std::ofstream& gpioFile, const char* stateFilePath,
                     bool detected, bool& initialized, bool& lastState) {
  if (initialized && detected == lastState) {
    return;
  }

  int gpioValue = detected ? kGpioDetectedValue : kGpioIdleValue;
  if (gpioFile.is_open()) {
    gpioFile << gpioValue << std::endl;
    gpioFile.flush();
  }
  if (stateFilePath && stateFilePath[0] != '\0') {
    std::ofstream stateFile(stateFilePath, std::ios::out | std::ios::trunc);
    if (stateFile.is_open()) {
      stateFile << gpioValue << std::endl;
    }
  }
  initialized = true;
  lastState = detected;
...
void updateHumanDetectionGpio(const std::shared_ptr<Yolov8Context>& context,
                              const common::ObjectMetadatas& objectMetadatas) {
  int personClassId = findPersonClassId(context);
  if (personClassId < 0) {
    return;
  }

  static std::ofstream valueFile429("/sys/class/gpio/gpio429/value");
  static std::ofstream valueFile430("/sys/class/gpio/gpio430/value");
  static bool channel20Initialized = false;
  static bool channel10Initialized = false;
  static bool channel20LastDetected = false;
  static bool channel10LastDetected = false;
  const int forcedChannelId = loadForcedChannelIdFromEnv();

  bool hasChannel20 = false;
  bool hasChannel10 = false;
  bool channel20Detected = false;
  bool channel10Detected = false;
  bool hasAnyFrame = false;
  bool anyChannelDetected = false;

  for (const auto& obj : objectMetadatas) {
    if (!obj || !obj->mFrame || obj->mFrame->mEndOfStream) {
      continue;
    }
    hasAnyFrame = true;

    bool humanDetectedThisFrame = false;
    for (const auto& detData : obj->mDetectedObjectMetadatas) {
      if (!detData || detData->mClassify != personClassId) {
        continue;
      }
      float score = detData->mScores.empty() ? 0.0f : detData->mScores[0];
      if (score >= context->thresh_conf_min) {
        humanDetectedThisFrame = true;
        break;
      }
    }
    anyChannelDetected = anyChannelDetected || humanDetectedThisFrame;

    if (obj->mFrame->mChannelId == kHumanChannelIdPrimary) {
      hasChannel20 = true;
      channel20Detected = channel20Detected || humanDetectedThisFrame;
    } else if (obj->mFrame->mChannelId == kHumanChannelIdSecondary) {
      hasChannel10 = true;
      channel10Detected = channel10Detected || humanDetectedThisFrame;
    }
  }

  if (forcedChannelId == kHumanChannelIdPrimary) {
    if (hasAnyFrame) {
      writeHumanState(valueFile429, kHumanStateFileChannel20, anyChannelDetected,
                      channel20Initialized, channel20LastDetected);
    }
    return;
  }
  if (forcedChannelId == kHumanChannelIdSecondary) {
    if (hasAnyFrame) {
      writeHumanState(valueFile430, kHumanStateFileChannel10, anyChannelDetected,
                      channel10Initialized, channel10LastDetected);
    }
    return;
  }

  if (hasChannel20) {
    writeHumanState(valueFile429, kHumanStateFileChannel20, channel20Detected,
                    channel20Initialized, channel20LastDetected);
  }
  if (hasChannel10) {
    writeHumanState(valueFile430, kHumanStateFileChannel10, channel10Detected,
                    channel10Initialized, channel10LastDetected);
  }
}
}  // namespace
...
void Yolov8PostProcess::postProcess(std::shared_ptr<Yolov8Context> context,
  ...
  if (context->taskType == TaskType::Detect) {
    ...
    updateHumanDetectionGpio(context, objectMetadatas);
