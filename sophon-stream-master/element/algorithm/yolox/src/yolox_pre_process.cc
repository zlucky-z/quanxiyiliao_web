  ...
  for (auto& objMetadata : objectMetadatas) {
    if (objMetadata->mFrame->mSpData != nullptr) {
      ...
      auto fail = [&](const char* stage, bm_status_t ret,
        ...
        std::cerr << "[YoloxPreProcess] " << stage << " failed, ret=" << ret
                  << std::endl;
        cleanup();
        return errorCode;
      };
      ...
      if (image0.image_format != jsonPlanner) {
        ...
        if (ret != BM_SUCCESS) {
          return fail("bm_image_alloc_dev_mem(image1)", ret,
      ...
      if (need_copy) {
        ...
        if (ret != BM_SUCCESS) {
          return fail("bm_image_alloc_dev_mem(image_aligned)", ret,
                      common::ErrorCode::ERR_STREAM_MEMORY_ALLOCATION);
        }
      ...
      if (ret != BM_SUCCESS) {
        return fail("bm_image_alloc_dev_mem(resized_img)", ret,
                    common::ErrorCode::ERR_STREAM_MEMORY_ALLOCATION);
      }
      ...
      if (context->roi_predefined) {
        if (context->roi.start_x > image1.width ||
          ...
          cleanup();
          return common::ErrorCode::ERR_OVER_BOUNDARY;
        }
      ...
      if (ret != BM_SUCCESS) {
        return fail("bm_malloc_device_byte(converto_img)", ret,
                    common::ErrorCode::ERR_STREAM_MEMORY_ALLOCATION);
      }
      input_mem = mem;
