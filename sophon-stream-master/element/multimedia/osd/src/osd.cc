...
std::vector<common::Point<int>> parseOverlayRegionPoints(
    const nlohmann::json& pointsJson) {
  ...
  for (const auto& pointJson : pointsJson) {
    ...
    const auto topIt = pointJson.find("top");
    const auto leftIt = pointJson.find("left");
    if (topIt == pointJson.end() || leftIt == pointJson.end() ||
        !topIt->is_number_integer() || !leftIt->is_number_integer()) {
      continue;
    }

    points.emplace_back(topIt->get<int>(), leftIt->get<int>());
  }
...
common::ErrorCode Osd::initInternal(const std::string& json) {
  ...
  do {
    ...
    mOverlayAreas.clear();
    mOverlayAreaLabels.clear();
    auto overlayRegionsIt = configure.find(CONFIG_INTERNAL_OVERLAY_REGIONS_FIELD);
    if (overlayRegionsIt != configure.end() && overlayRegionsIt->is_array()) {
      for (const auto& regionJson : *overlayRegionsIt) {
        if (!regionJson.is_object()) {
          continue;
        }

        std::vector<common::Point<int>> points = parseOverlayRegionPoints(
            regionJson.value("points", nlohmann::json::array()));
        if (points.size() < 2) {
          continue;
        }

        mOverlayAreas.push_back(points);
        mOverlayAreaLabels.push_back(regionJson.value("label", ""));
      }
    }
    ...
    if (mOsdType == OsdType::ALGORITHM) {
               ...
               "draw_retinaface_distributor_resnet_faiss_converger_results")
      {
        ...
        draw_func_opencv = std::bind(
            draw_retinaface_distributor_resnet_faiss_converger_results_opencv,
            std::placeholders::_1, std::placeholders::_2);
      }
    ...
    switch (mOsdType) {
      ...
      case OsdType::TRACK:
        draw_opencv_track_result(objectMetadata, mClassNames, frame_to_draw,
                                 mPutText, mDrawInterval);
        if (!mOverlayAreas.empty()) {
          draw_opencv_configured_areas(mOverlayAreas, mOverlayAreaLabels,
                                       frame_to_draw);
        }
        break;
    ...
    switch (mOsdType) {
      ...
      case OsdType::TRACK:
        draw_bmcv_track_result(objectMetadata->mFrame->mHandle, objectMetadata,
                               ...
                               mDrawInterval);
        if (!mOverlayAreas.empty()) {
          draw_bmcv_configured_areas(objectMetadata->mFrame->mHandle,
                                     mOverlayAreas, mOverlayAreaLabels,
                                     *imageStorage);
        }
        break;
