...
const cv::Scalar kAreaOutlineColorCv = cv::Scalar(0, 255, 255);
const bmcv_color_t kAreaOutlineColorBmcv = {255, 255, 0};
constexpr int kAreaLabelColorR = 255;
constexpr int kAreaLabelColorG = 255;
constexpr int kAreaLabelColorB = 0;
const cv::Scalar kMatchedTrackColorCv = cv::Scalar(0, 0, 255);
const bmcv_color_t kMatchedTrackColorBmcv = {255, 0, 0};
...
struct FaceDrawCacheEntry {
  std::int64_t frame_id = -1;
  std::vector<bmcv_rect_t> rects;
  std::vector<std::string> labels;
};
std::map<int, FaceDrawCacheEntry> lastFaceDrawCacheMap;
std::mutex mLastFaceDrawCacheMtx;
std::mutex mUtf8TextDrawMtx;
std::string mUtf8TextFontPath;
bool mUtf8TextFontWarned = false;
std::shared_ptr<uni_text::UniText> mUtf8TextRenderer;
std::unordered_map<std::string, std::shared_ptr<bm_image>> mUtf8TextBitmapCache;
const size_t kUtf8TextBitmapCacheLimit = 256;
...
inline std::string resolveUtf8FontPath() {
  if (!mUtf8TextFontPath.empty()) {
    return mUtf8TextFontPath;
  }
  const std::vector<std::string> candidates = {
      "/data/sophon-stream-master/samples/ppocr/data/wqy-microhei.ttc",
      "/data/sophon-stream/samples/ppocr/data/wqy-microhei.ttc",
      "../ppocr/data/wqy-microhei.ttc",
      "../license_plate_recognition/data/wqy-microhei.ttc"};
  struct stat st;
  for (const auto& path : candidates) {
    if (stat(path.c_str(), &st) == 0 && S_ISREG(st.st_mode)) {
      mUtf8TextFontPath = path;
      break;
    }
  }
  return mUtf8TextFontPath;
...
inline std::shared_ptr<bm_image> getUtf8Bitmap(bm_handle_t& handle,
                                               const std::string& text,
                                               int r = 255, int g = 0,
                                               int b = 0) {
  const std::string cacheKey =
      text + "#" + std::to_string(r) + "," + std::to_string(g) + "," +
      std::to_string(b);
  auto cacheIt = mUtf8TextBitmapCache.find(cacheKey);
  if (cacheIt != mUtf8TextBitmapCache.end()) {
    return cacheIt->second;
  }

  std::string fontPath = resolveUtf8FontPath();
  if (fontPath.empty()) {
    if (!mUtf8TextFontWarned) {
      IVS_WARN("No Chinese font found for UTF-8 OSD text, text: {0}", text);
      mUtf8TextFontWarned = true;
    }
    return nullptr;
  }

  if (!mUtf8TextRenderer) {
    mUtf8TextRenderer = std::make_shared<uni_text::UniText>(fontPath, 28);
  }

  std::shared_ptr<bm_image> overlay(new bm_image, [](bm_image* img) {
    if (img) {
  ...
  if (!mUtf8TextRenderer->genBitMap(handle, text, *overlay, r, g, b)) {
    return nullptr;
  }

  if (mUtf8TextBitmapCache.size() >= kUtf8TextBitmapCacheLimit) {
    mUtf8TextBitmapCache.clear();
  }
  mUtf8TextBitmapCache[cacheKey] = overlay;
  return overlay;
}

inline std::string buildTrackLabel(
    const std::shared_ptr<common::TrackedObjectMetadata>& trackObj) {
  if (trackObj == nullptr) {
    return "";
  }

  std::string label = std::to_string(trackObj->mTrackId);
  if (!trackObj->mName.empty()) {
    label += " ";
    label += trackObj->mName;
  }
  return label;
}

inline bool shouldHighlightMatchedTrack(
    const std::shared_ptr<common::TrackedObjectMetadata>& trackObj) {
  return trackObj != nullptr && trackObj->mRegisteredMatched;
}

inline std::string resolveAreaLabel(const std::vector<std::string>* labels,
    size_t areaIndex) {
  if (labels == nullptr || areaIndex >= labels->size()) {
    return "";
  }
  return (*labels)[areaIndex];
}

inline cv::Point getAreaLabelAnchorCv(
    const std::vector<common::Point<int>>& points) {
  if (points.empty()) {
    return cv::Point(0, 0);
  }

  int minTop = points[0].mX;
  int minLeft = points[0].mY;
  for (size_t i = 1; i < points.size(); ++i) {
    minTop = std::min(minTop, points[i].mX);
    minLeft = std::min(minLeft, points[i].mY);
  }

  return cv::Point(std::max(minLeft, 0), std::max(minTop - 10, 18));
}

inline bmcv_point_t getAreaLabelAnchorBmcv(
    const std::vector<common::Point<int>>& points) {
  if (points.empty()) {
    return bmcv_point_t{0, 24};
  }
  ...
  return bmcv_point_t{std::max(minLeft, 0), std::max(minTop - 18, 0)};
}

static void _draw_text_bmcv(
    bm_handle_t& handle, int left, int top, bm_image& frame,
    std::string label, int r = 255, int g = 0, int b = 0);

static bool _draw_text_overlay_or_put_text_bmcv(
    bm_handle_t& handle, int baseline_x, int baseline_y, bm_image& frame,
    const std::string& label, int r = 255, int g = 0, int b = 0);

void draw_opencv_areas_impl(
    const std::vector<std::vector<common::Point<int>>>& areas,
    const std::vector<std::string>* labels, cv::Mat& frame_to_draw) {
  const cv::Scalar color = kAreaOutlineColorCv;
  ...
  for (size_t i = 0; i < areas.size(); ++i) {
    ...
    polygonPoints.reserve(area.size());
    for (size_t pointIndex = 0; pointIndex < area.size(); ++pointIndex) {
      polygonPoints.emplace_back(area[pointIndex].mY, area[pointIndex].mX);
    }

    if (polygonPoints.size() >= 3) {
      cv::polylines(frame_to_draw, polygonPoints, true, color, thickness, 8, 0);
    } else {
      cv::line(frame_to_draw, polygonPoints[0], polygonPoints[1], color,
               thickness, 8, 0);
    }

    const std::string label = resolveAreaLabel(labels, i);
    if (!label.empty()) {
      cv::putText(frame_to_draw, label, getAreaLabelAnchorCv(area),
...
void draw_opencv_areas(std::shared_ptr<common::ObjectMetadata> objectMetadata,
  ...
  draw_opencv_areas_impl(objectMetadata->areas, nullptr, frame_to_draw);
}

void draw_opencv_configured_areas(
    const std::vector<std::vector<common::Point<int>>>& areas,
    const std::vector<std::string>& labels, cv::Mat& frame_to_draw) {
  draw_opencv_areas_impl(areas, &labels, frame_to_draw);
}

void draw_bmcv_areas_impl(
    bm_handle_t& handle,
    const std::vector<std::vector<common::Point<int>>>& areas,
    const std::vector<std::string>* labels, bm_image& frame_to_draw) {
  bmcv_color_t color = kAreaOutlineColorBmcv;
  const int thickness = 2;
  ...
  for (size_t i = 0; i < areas.size(); ++i) {
    ...
    for (size_t pointIndex = 0; pointIndex < area.size(); ++pointIndex) {
      const auto& startPoint = area[pointIndex];
      const auto& endPoint =
          area[(pointIndex + 1) % area.size()];
      if (area.size() == 2 && pointIndex > 0) {
        break;
      }

      bmcv_point_t start = {startPoint.mY, startPoint.mX};
      bmcv_point_t end = {endPoint.mY, endPoint.mX};
      bmcv_image_draw_lines(handle, frame_to_draw, &start, &end, 1, color,
                            thickness);
    }

    const std::string label = resolveAreaLabel(labels, i);
    if (!label.empty()) {
      bmcv_point_t anchor = getAreaLabelAnchorBmcv(area);
      _draw_text_bmcv(handle, anchor.x, anchor.y, frame_to_draw, label,
                      kAreaLabelColorR, kAreaLabelColorG, kAreaLabelColorB);
    }
  }
}
...
void draw_bmcv_areas(std::shared_ptr<common::ObjectMetadata> objectMetadata,
  ...
  draw_bmcv_areas_impl(handle, objectMetadata->areas, nullptr, frame_to_draw);
}

void draw_bmcv_configured_areas(
    bm_handle_t& handle,
    const std::vector<std::vector<common::Point<int>>>& areas,
    const std::vector<std::string>& labels, bm_image& frame_to_draw) {
  draw_bmcv_areas_impl(handle, areas, &labels, frame_to_draw);
}
...
void draw_bmcv_det_result(
  ...
  if (put_text_flag) {
    for (auto detObj : objectMetadata->mDetectedObjectMetadatas) {
      ...
      _draw_text_overlay_or_put_text_bmcv(handle, org_x, org_y, frame, label,
                                          255, 0, 0);
    }
  }
}
...
void draw_bmcv_track_result(
  ...
  for (auto detObj : objData->mDetectedObjectMetadatas) {
    ...
    rect.crop_h = detObj->mBox.mHeight;
    auto trackObj =
        idx < objData->mTrackedObjectMetadatas.size()
            ? objData->mTrackedObjectMetadatas[idx]
            : nullptr;
    int track_id = trackObj ? trackObj->mTrackId : idx;
    ...
    for (auto detObj : objData->mDetectedObjectMetadatas) {
      auto trackObj =
          idx < objData->mTrackedObjectMetadatas.size()
              ? objData->mTrackedObjectMetadatas[idx]
              : nullptr;
      std::string label = buildTrackLabel(trackObj);
      if (!label.empty()) {
        _draw_text_bmcv(handle, detObj->mBox.mX, detObj->mBox.mY - 40, frame,
                        label);
      }
      ++idx;
    }
  ...
  for (auto detObj : objData->mDetectedObjectMetadatas) {
    auto trackObj =
        idx < objData->mTrackedObjectMetadatas.size()
            ? objData->mTrackedObjectMetadatas[idx]
            : nullptr;
    if (shouldHighlightMatchedTrack(trackObj)) {
      bmcv_rect_t rect;
      rect.start_x = detObj->mBox.mX;
      rect.start_y = detObj->mBox.mY;
      rect.crop_w = detObj->mBox.mWidth;
      rect.crop_h = detObj->mBox.mHeight;
      bmcv_image_draw_rectangle(handle, frame, 1, &rect, thickness,
                                kMatchedTrackColorBmcv.r,
                                kMatchedTrackColorBmcv.g,
                                kMatchedTrackColorBmcv.b);
      if (put_text_flag) {
        std::string label = buildTrackLabel(trackObj);
        if (!label.empty()) {
          _draw_text_bmcv(handle, detObj->mBox.mX, detObj->mBox.mY - 40, frame,
                          label, kMatchedTrackColorBmcv.r,
                          kMatchedTrackColorBmcv.g, kMatchedTrackColorBmcv.b);
        }
      }
    }
    ++idx;
...
void draw_opencv_track_result(
  ...
  draw_opencv_areas(objData, frame);
  ...
  for (auto detObj : objData->mDetectedObjectMetadatas) {
    ...
    int track_id = trackObj ? trackObj->mTrackId : idx;
    cv::Scalar color =
        shouldHighlightMatchedTrack(trackObj)
            ? kMatchedTrackColorCv
            : cv::Scalar(colors[track_id % colors_num][0],
    ...
    bool put_text_flag)  // Draw the predicted bounding box
{
  ...
  int ret = bmcv_image_draw_rectangle(handle, frame, 1, &rect, 3, color[0],
                                      color[1], color[2]);
  if (put_text_flag) {
    _draw_text_overlay_or_put_text_bmcv(handle, left, std::max(top - 10, 0),
                                        frame, lable, color[0], color[1],
                                        color[2]);
  }
}
...
static void _draw_text_bmcv(
    bm_handle_t& handle, int left, int top, bm_image& frame,
    std::string label, int r, int g,
    int b)  // Draw the predicted bounding box
{
  const int textX = std::max(left, 0);
  const int textBaselineY = std::max(top + 40, 0);

  _draw_text_overlay_or_put_text_bmcv(handle, textX, textBaselineY, frame,
                                      label, r, g, b);
}

static bool _draw_text_overlay_or_put_text_bmcv(
    bm_handle_t& handle, int baseline_x, int baseline_y, bm_image& frame,
    const std::string& label, int r, int g, int b) {
  const int textX = std::max(baseline_x, 0);
  const int textBaselineY = std::max(baseline_y, 0);

  if (bmcv_image_overlay != nullptr) {
    std::lock_guard<std::mutex> lk(mUtf8TextDrawMtx);
    auto overlay = getUtf8Bitmap(handle, label, r, g, b);
    const int frameWidth = static_cast<int>(frame.width);
    const int frameHeight = static_cast<int>(frame.height);
    if (overlay && overlay->width > 0 && overlay->height > 0 &&
        frameWidth > 0 && frameHeight > 0 &&
        overlay->width <= frameWidth && overlay->height <= frameHeight) {
      int overlayLeft = std::max(0, std::min(textX, frameWidth - overlay->width));
      int overlayTop = std::max(
          0, std::min(textBaselineY - overlay->height, frameHeight - overlay->height));
      bmcv_rect_t overlayInfo = {overlayTop, overlayLeft, overlay->height,
                                 overlay->width};
      bmcv_image_overlay(handle, frame, 1, &overlayInfo, overlay.get());
      return true;
    }
  }

  bmcv_point_t org = {textX, textBaselineY};
  bmcv_color_t bmcv_color = {r, g, b};
  int thickness = 2;
  float fontScale = 1.5;
...
// 在同一个文件中，确保已经声明了以下辅助函数：
// void _draw_text_bmcv(bm_handle_t& handle, int x, int y, bm_image& frame, const std::string& text);
// （或包含对应的头文件）
...
void draw_retinaface_distributor_resnet_faiss_converger_results(
  ...
  const std::int64_t frame_id = objectMetadata->mFrame->mFrameId;
  constexpr std::int64_t kFaceCacheMaxAgeFrames = 2;
  std::vector<bmcv_rect_t> rects;
  std::vector<std::string> labels;

  if (!objectMetadata->mFaceObjectMetadatas.empty()) {
    rects.reserve(objectMetadata->mFaceObjectMetadatas.size());
    labels.assign(objectMetadata->mFaceObjectMetadatas.size(), "");
    for (const auto &faceObj : objectMetadata->mFaceObjectMetadatas) {
      bmcv_rect_t rect;
      rect.start_x = std::max(faceObj->left, 0);
      rect.start_y = std::max(faceObj->top, 0);
      rect.crop_w = std::max(faceObj->right - faceObj->left + 1, 1);
      rect.crop_h = std::max(faceObj->bottom - faceObj->top + 1, 1);
      rects.push_back(rect);
    }

    for (const auto &subObj : objectMetadata->mSubObjectMetadatas) {
      int subId = subObj->mSubId;
      if (subId < 0 || static_cast<size_t>(subId) >= labels.size()) {
        continue;
      }
      if (subObj->mRecognizedObjectMetadatas.empty()) {
        continue;
      }
      labels[subId] = subObj->mRecognizedObjectMetadatas[0]->mLabelName;
    }

    std::lock_guard<std::mutex> lk(mLastFaceDrawCacheMtx);
    lastFaceDrawCacheMap[channel_id] = FaceDrawCacheEntry{frame_id, rects, labels};
  } else {
    std::lock_guard<std::mutex> lk(mLastFaceDrawCacheMtx);
    auto it = lastFaceDrawCacheMap.find(channel_id);
    if (it != lastFaceDrawCacheMap.end()) {
      bool cacheValid =
          frame_id >= it->second.frame_id &&
          frame_id - it->second.frame_id <= kFaceCacheMaxAgeFrames;
      if (cacheValid) {
        rects = it->second.rects;
        labels = it->second.labels;
      } else {
        lastFaceDrawCacheMap.erase(it);
      }
    }
  }
  ...
  bmcv_image_draw_rectangle(handle, frame, rects.size(), rects.data(), 2, 255,
                            2, 2);
  ...
  for (size_t i = 0; i < rects.size(); ++i) {
    ...
    const int rectStartX = static_cast<int>(rects[i].start_x);
    const int rectStartY = static_cast<int>(rects[i].start_y);
    const int callTop = std::max(rectStartY - TEXT_INTERNAL_OFFSET, 0);
    _draw_text_bmcv(handle, rectStartX, callTop, frame, labels[i]);
  }
}

void draw_retinaface_distributor_resnet_faiss_converger_results_opencv(
    std::shared_ptr<sophon_stream::common::ObjectMetadata> objectMetadata,
    cv::Mat& frame) {
  const int channel_id = objectMetadata->mFrame->mChannelId;
  const std::int64_t frame_id = objectMetadata->mFrame->mFrameId;
  ...
  const cv::Scalar boxColor(2, 2, 255);
  for (size_t i = 0; i < rects.size(); ++i) {
    const cv::Rect rect(rects[i].start_x, rects[i].start_y, rects[i].crop_w,
                        rects[i].crop_h);
    cv::rectangle(frame, rect, boxColor, 2);
    if (i >= labels.size() || labels[i].empty()) {
      continue;
    }
    const int textY = std::max(rect.y - kTextOffset, 20);
    cv::putText(frame, labels[i], cv::Point(rect.x, textY),
