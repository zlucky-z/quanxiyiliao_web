...
class Osd : public ::sophon_stream::framework::Element {
 public:
  enum class OsdType { DET, TRACK, REC, POSE, AREA, OBB, ALGORITHM, TEXT, UNKNOWN };
  ...
  static constexpr const char* CONFIG_INTERNAL_OVERLAY_REGIONS_FIELD =
      "overlay_regions";

 private:
  std::vector<std::string> mClassNames;
  std::vector<std::string> mRecognizeNames;
  std::vector<std::vector<common::Point<int>>> mOverlayAreas;
  std::vector<std::string> mOverlayAreaLabels;
