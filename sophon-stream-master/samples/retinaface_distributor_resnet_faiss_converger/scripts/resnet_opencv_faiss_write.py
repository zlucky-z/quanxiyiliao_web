...
def main(args):
        ...
        if os.path.isdir(folder_path):
            ...
            for filename in os.listdir(folder_path):
                ...
                if len(img_list) == batch_size:
                    feature_vectors = resnet(img_list)
                    for i, filename in enumerate(filename_list):
                       parts = filename.split('/')
                       label = parts[-2]
                       print("real:"+label)
                       feature_rows.append(np.array(feature_vectors[i], copy=True))
    ...
    # 兼容旧版 libfaiss 固定 sort_cnt=5：数据库向量不足5时会报错。
    # 这里按已有特征循环补齐到5条，避免检索阶段直接失败。
    if 0 < num_Sum < 5:
        pad_count = 5 - num_Sum
        for i in range(pad_count):
            src_index = i % num_Sum
            feature_rows.append(np.array(feature_rows[src_index], copy=True))
            label_rows.append(label_rows[src_index])
        logging.warning("feature rows (%d) < 5, padded to %d for faiss compatibility.", num_Sum, len(feature_rows))

    db_data = open(db_data_path, 'a')
    index_label = open(index_label_path, 'a')
    for feature_vector, label in zip(feature_rows, label_rows):
...
def argsparser():
    ...
    parser.add_argument('--input', type=str, default='/data/sophon-stream-master/samples/retinaface_distributor_resnet_faiss_converger/data/images/face_data_train', help='path of input, must be image directory')
    parser.add_argument('--bmodel', type=str, default='/data/sophon-stream-master/samples/retinaface_distributor_resnet_faiss_converger/data/models/BM1688/resnet_arcface_fp32_1b.bmodel', help='path of bmodel')
