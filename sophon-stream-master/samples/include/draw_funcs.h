...
static void _gen_storage_image(
...
void draw_license_plate_recognition_results(
    std::shared_ptr<sophon_stream::common::ObjectMetadata> objectMetadata,
    std::string& out_dir) {
  bm_image imageStorage;
  _gen_storage_image(objectMetadata, imageStorage);
...
void draw_openpose_results(
  ...
  if (image.image_format == 11) {
    ...
    bm_image_create(objectMetadata->mFrame->mHandle, image.height, image.width,
                    FORMAT_YUV420P, image.data_type, &imageStorage);
    bmcv_rect_t crop_rect = {0, 0, image.width, image.height};
    bmcv_image_vpp_convert(objectMetadata->mFrame->mHandle, 1, image_aligned,
...
void draw_retinaface_distributor_resnet_faiss_converger_results(
  ...
  if (!objectMetadata->mFaceObjectMetadatas.empty()) {
    ...
    for (auto subObj : objectMetadata->mSubObjectMetadatas) {
      if (subObj == nullptr || subObj->mRecognizedObjectMetadatas.empty()) {
        continue;
      }
      int subId = subObj->mSubId;
      if (subId < 0 ||
          static_cast<size_t>(subId) >= objectMetadata->mFaceObjectMetadatas.size()) {
        continue;
      }

      auto faceObj = objectMetadata->mFaceObjectMetadatas[subId];
      if (faceObj == nullptr) {
        continue;
      }

      auto recognizedObj = subObj->mRecognizedObjectMetadatas[0];
      if (recognizedObj == nullptr) {
        continue;
      }

      auto label = recognizedObj->mLabelName;
      if (label.empty()) {
        continue;
      }

      _draw_text_bmcv(objectMetadata->mFrame->mHandle, std::max(faceObj->left, 0),
                      std::max(faceObj->top, 0), imageStorage, label);
    }
...
void draw_yolov5_fastpose_posec3d_results(
    std::shared_ptr<sophon_stream::common::ObjectMetadata> objectMetadata,
    std::string& out_dir, std::string loss_type) {
  bm_image imageStorage;
  _gen_storage_image(objectMetadata, imageStorage);
  if (objectMetadata->mRecognizedObjectMetadatas.size() != 0)
    _draw_text_bmcv(
        objectMetadata->mFrame->mHandle, 50, 10, imageStorage,
